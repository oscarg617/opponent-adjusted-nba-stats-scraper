...
def scrape(url):
    for _ in range(MAX_RETRIES):
        ...
        if response.ok:  # If we get a successful request
            ...
            data_pd = data_pd.iloc[
                :,
                [len(data_pd.columns) - 1]
                + [x for x in range(len(data_pd.columns) - 1)],
            ]
            data_pd = data_pd[
                (data_pd["AST"].str.contains("Inactive") == False)
                & (data_pd["AST"].str.contains("AST") == False)
                & (data_pd["AST"].str.contains("Did Not Play") == False)
                & (data_pd["AST"].str.contains("Did Not Dress") == False)
            ].rename(
                columns={
