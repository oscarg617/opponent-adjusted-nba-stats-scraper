...
client = ScrapingBeeClient(api_key='YIX02QIVSSSNUFEHSIFMAEA3W0SVDEXKUR9E6TLSRUS8EO54VV39E16A213PFLM9D5FIZ4CZ7GPJW4PM')
...
def scrape_defense(year):
    ...
    if response.ok:
        ...
        teams_lst = [a.get("href")[7:10] for a in [td for td in [td.a for td in teams_lst] if td is not None]]
        drtg_lst =  [float(td.text) for td in table.find_all("td", attrs={"class": "right", "data-stat": "def_rtg"})]
        table_ts_pct = soup.find("table", attrs={"id": "totals-opponent"})
        teams_lst2 = table_ts_pct.find_all("td", attrs={"class": "left", "data-stat": "team"})
        teams_lst2 = [a.get("href")[7:10] for a in [td for td in [td.a for td in teams_lst2] if td is not None]]
        fga = [int(td.text) for td in table_ts_pct.find_all("td", attrs={"class": "right", "data-stat": "opp_fga"})]
        fta = [int(td.text) for td in table_ts_pct.find_all("td", attrs={"class": "right", "data-stat": "opp_fta"})]
        pts = [int(td.text) for td in table_ts_pct.find_all("td", attrs={"class": "right", "data-stat": "opp_pts"})]
        ...
        defense_tuples = [(ts_tuples[i][0], drtg_tuples[i][1], ts_tuples[i][1]) for i in range(len(ts_tuples))]
